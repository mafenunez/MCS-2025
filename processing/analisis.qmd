...
#valores predichos variable cuanti 
ggeffects::ggpredict(fit04, terms="esfuerzo") %>%
  ggplot(mapping=aes(x = x, y=predicted)) +
...
#valores predichos variable cuanti 
ggeffects::ggpredict(fit05, terms="talento") %>%
  ggplot(mapping=aes(x = x, y=predicted)) +
...
variables de meritocracia con estatus social subjetivo

```{r}
#| echo: false
#| results: asis

m3.1log <- glm(just_educacion_recod~genero+ingreso_hogar+estatus_subj*esfuerzo+talento,data=proc_data,family = "binomial")

m3.2log <- glm(just_educacion_recod~genero+ingreso_hogar+estatus_subj*talento+esfuerzo,data=proc_data,family = "binomial")
```
...
```{r}
htmlreg(list(m1log,m2log,m3log,m3.2log),
          custom.model.names = c("M1 (log odds)","M2 (log odds)","M3 (log odds)",
                                 "M3.2 (log odds)"),
          digits = 3, 
          stars = c(0.001, 0.01, 0.05, 0.1),symbol = "†")
...
FigSindSector_int<-ggeffects::ggpredict(m3.1log, terms = c("estatus_subj", "esfuerzo")) %>%
...
```{r}
# ojo que la relación sindicalización x sector privado no es significativa
FigSindSector_int<-ggeffects::ggpredict(m3.2log, terms = c("estatus_Subj", "talento")) %>%
  ggplot(aes(x=x, y=predicted, shape = group, color = group)) +
  geom_line(aes(group=group,linetype = group),position = position_dodge(.1)) + 
  geom_point(size = 2.5,position = position_dodge(.1))+
  scale_x_continuous(name = "", breaks=c(0,1), labels = c("No sindicalizados/as", "Sindicalizados/as")) + 
