...
Además, se reocdifico en dos cateogrias la variable dependiente, para ver sus resultados utilizando regresión logisitca. Donde 0 es "En desacuerdo" y 1 es "De acuerdo". Dentro de la recodificación, se considero como NA la cateogoría "Ni de acuerdo ni en desacuerdo", lo cual conlleva la perdida de 228 casos, contando con una segunda base con un N de 2238 casos. 
...
En general, se observa que las correlaciones entre las variables indepedientes son de intensidad baja y significativas (expecto en el caso de los criterios de meritocracia). 


Ahora, veremos como se relaciona cada variable predictora con las preferencias por justicia de mercado en la eduación de acuerdo a sus niveles de medición. 
...
### Valores predichos: Estatus social subjetivo

```{r}
#| echo: false

#valores predichos variable cuanti 
ggeffects::ggpredict(fit03, terms="estatus_subj") %>%
  ggplot(mapping=aes(x = x, y=predicted)) +
  labs(title="Estatus Social Subjetivo", x = "", y = "")+
  theme_bw() +
 geom_smooth()+
...
pred <- ggeffects::ggpredict(fit04, terms = c("esfuerzo"))


ggeffects::ggpredict(fit04, terms = c("esfuerzo")) %>%
  ggplot(aes(x=x, y=predicted)) +
  geom_bar(stat="identity", color="grey", fill="grey")+
  geom_errorbar(aes(ymin = conf.low, ymax = conf.high), width=.1) +
...
pred <- ggeffects::ggpredict(fit05, terms = c("talento"))


ggeffects::ggpredict(fit05, terms = c("talento")) %>%
  ggplot(aes(x=x, y=predicted)) +
  geom_bar(stat="identity", color="grey", fill="grey")+
  geom_errorbar(aes(ymin = conf.low, ymax = conf.high), width=.1) +
...
FigSind_1_Prob <- ggeffects::ggpredict(m3log, terms = c("genero")) %>%
  ggplot(aes(x=x, y=predicted)) +
  geom_bar(stat="identity", color="grey", fill="grey")+
  geom_errorbar(aes(ymin = conf.low, ymax = conf.high), width=.1) +
  labs(title="Género", x = "", y = "") +
